...
function SecondaryProductGrid({title, items}: {title: string, items: Product[]}) {
  const recommendations = [...new Map((items as any).map((item: any) => [item.productId, item])).values()] as unknown as Product[];
  const slides = recommendations.map((item, idx) => (
    <div key={`${item.productId}_${idx}`} className="shrink-0 mb-4">
...
export default function RecGrid() {
  ...
  const productIds = cart.map(p => `productIds=${p.productId}`).join("&");
  const ctx = {userId, productIds}
  return (
    <div className="mb-6 space-y-6">
      <TypeToggle toggleOn={toggleOn} setToggleOn={setToggleOn} />
      {/*<p className="italic text-xs">You often buy these together!</p>*/}
      {/*<p className="italic text-xs">Other shoppers add this next!</p>*/}
      {/*<p className="italic text-xs">You might need this for a recipe!</p>*/}
      {toggleOn
        ? (
          <div>
            {/*<strong>Pros:</strong>*/}
            {/*<ul>*/}
            {/*  <li>✅ Works even if a product has no purchase history (cold start problem).</li>*/}
            {/*  <li>✅ Captures latent patterns in user preferences (e.g., related product categories).</li>*/}
            {/*  <li>✅ More personalized than basic collaborative filtering.</li>*/}
            {/*</ul>*/}
            {/*<strong>Cons:</strong>*/}
            {/*<ul>*/}
            {/*  <li>❌ Harder to explain why a recommendation was made.</li>*/}
            {/*  <li>❌ Requires vector indexing setup in Neo4j.</li>*/}
            {/*</ul>*/}
            <RecSection
              title={userId > 0 ? "You often buy these together! (ai)" : "Frequently bought together! (ai)"}
              endpoint="/api/recs/ai/bought-together"
              {...ctx}
            />
            {/*<RecSection*/}
            {/*  title="Similar shoppers add this next! (ai)"*/}
            {/*  endpoint="/api/recs/similar-user-products"*/}
            {/*  {...ctx}*/}
            {/*/>*/}
          </div>
        )
        : (
          <div>
            ...
            {/*<strong>Pros:</strong>*/}
            {/*<ul>*/}
            {/*  <li>✅ Simple and explainable.</li>*/}
            {/*  <li>✅ Works well when there’s sufficient co-purchase data.</li>*/}
            {/*  <li>✅ No AI model dependency—purely graph-based.</li>*/}
            {/*</ul>*/}
            {/*<strong>Cons:</strong>*/}
            {/*<ul>*/}
            {/*  <li>❌ Won’t recommend new products with little purchase history.</li>*/}
            {/*  <li>❌ Doesn’t account for individual user preferences.</li>*/}
            {/*</ul>*/}
            <RecSection
              title={userId > 0 ? "You often buy these together! (graph)" : "Frequently bought together! (graph)"}
              endpoint="/api/recs/std/bought-together"
              {...ctx}
            />
            {/*<RecSection title="Similar shoppers add this next! (graph)" endpoint="/api/recs/similar-user-products" {...ctx} />*/}
          </div>
        )
      }

      {/*<RecSection title={userId > 0 ? "You often buy these together!" : "Frequently bought together!"} endpoint="/api/recs/std/bought-together" {...ctx} />*/}
      {/*<RecSection title="Similar shoppers add this next!" endpoint="/api/recs/similar-user-products" {...ctx} />*/}
      {/*<SimilarUserProducts productIds={productIds} userId={userId} />*/}
      {/*{userId > 0 &&*/}
      {/*  <BoughtTogether productIds={productIds} userId={userId} />}*/}
    </div>
  )
}
